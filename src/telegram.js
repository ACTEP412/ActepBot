...
class TelegramBot {
    ...
    async onMessage(ctx) {
        try {
            ...
            if(!this.isUserAuthed(ctx) && msg == global.settings.token) {
                ...
                ctx.reply('Я взломал тебя! Шучу брат. Теперь ты авторизован и можешь управлять ботом.', this.mainKeyboard.reply());
                return;
            }
            
            if(!this.isUserAuthed(ctx)) {
                ctx.reply('Пришли мне golden_key, который ты вводил при настройке бота, чтобы начать работу.');
                return;
            }
            ...
            ctx.reply(`Братан, я словил ошибку... Незнаю как так получилось, но вот всё, что мне известно: ${err}`, this.mainKeyboard.reply());
        }
    }
    ...
    getInfo(ctx) {
        const msg = `❔ <b>ActepBot</b> ❔\n\n<b>ActepBot</b> - это бот для площадки funpay.com с открытым исходным кодом, разработанный <b>ACTEP412</b>.\n\nСпасибо всем за вашу поддержку.\n\n<a href="https://github.com/ACTEP412/ActepBot">GitHub</a> | <a href="https://github.com/ACTEP412/ActepBot"></a>`;
    ...
    addProductName(ctx) {
        ctx.replyWithHTML(`Окей, отправь мне название предложения. Можешь просто скопировать его из funpay. Эмодзи в названии поддерживаются.`);
    ...
    async deleteLot(ctx) {
        ...
        ctx.reply(`Окей, удалил товар "${name}" из списка автовыдачи.`, this.mainKeyboard.reply());
    }
